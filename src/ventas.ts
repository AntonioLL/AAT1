// Datos de los productos (en una base de datos real, estos vendrían de una API)
type Product = {
    id: number;
    name: string;
    price: number;
  ...
  function addToCart(productId: number) {
    ...
    if (product && product.stock > 0) {
      ...
      console.log(`Lo siento, no hay más ${product?.name} en stock.`);
    }
  }
  ...
  function updateCartItems() {
    ...
    if (cartItemsElement) {
      ...
      cart.forEach(product => {
        ...
        cartItemElement.textContent = `${product.name} - $${product.price.toFixed(2)}`;
